...
describe('tests button', () => {
	...
	describe('setup', () => {
		it('should the component has one event', () => {
			expect(docButton.setup && docButton.setup.length).toEqual(1)
		})

		it('should the component has event, it called success', () => {
			expect(getTestDescriptor(docButton.setup, 'success').properties).toMatchInlineSnapshot(`undefined`)
		})

		it('should the description of success event is Success event.', () => {
			expect(getTestDescriptor(docButton.setup, 'success').description).toBe('Success event.')
		})
	})
